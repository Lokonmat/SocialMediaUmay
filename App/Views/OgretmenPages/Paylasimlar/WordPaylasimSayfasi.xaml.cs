...
namespace App.Views.OgretmenPages.Paylasimlar
{
    ...
    public partial class WordPaylasimSayfasi : ContentPage
    {
        string id = Preferences.Get("Id", "");
        string fullName = Preferences.Get("FullName", "");
        private string selectedWordPath;
        ...
        private async void PickWord_Clicked(object sender, EventArgs e)
        {
            ...
            if (fileResult != null)
            {
                ...
                if (extension == ".docx" || extension == ".doc")
                {
                    ...
                    selectedWordPath = fileResult.FullPath;
                    selectedWordName = fileResult.FileName;
                    wordNameLabel.Text = $"{fileResult.FileName}";
                    wordFrame.IsVisible = true;
                }
                else
                {
                    // Geçerli bir Word belgesi seçilmediği durumunda kullanıcıya bilgi verin
                    await DisplayAlert("Hata", "Lütfen bir Word belgesi seçin.", "Tamam");
                }
            }
        }
